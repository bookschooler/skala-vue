...
const skyColorOptions = [
  ...
  { label: '노을 주황', value: '#F2A65A' },
  { label: '노을 분홍', value: '#E88D9C' },
  { label: '해질녘 보랏빛', value: '#9275B5' },
]
const cloudAmountOptions = ['없음', '조금', '보통', '많음', '가득']
const hourOptions = Array.from({ length: 12 }, (_, index) => String(index + 1))
const minuteOptions = Array.from({ length: 12 }, (_, index) => String(index * 5).padStart(2, '0'))
const profileImageOptions = [
  '/img/community-sky-day.png',
  '/img/community-sky-altocumulus.png',
  '/img/community-sky-sun-cloud.png',
  '/img/community-sky-rain-window.png',
  '/img/community-sky-sunset.png',
  '/img/community-sky-dusk.png',
]
...
const visiblePosts = computed(() =>
  communityPostStore.posts.filter((post) =>
    ...
    && (!filterState.skyColor || getPostSkyColors(post).includes(filterState.skyColor))
...
function getPostSkyColors(post) {
  return Array.isArray(post.skyColors) ? post.skyColors : post.skyColor ? [post.skyColor] : []
}

function getPostSkyColorLabel(post) {
  return getPostSkyColors(post).map(getSkyColorLabel).join(' · ')
}

function getProfileImage(name) {
  const value = String(name ?? '하늘지기')
  const seed = [...value].reduce((total, character) => total + character.charCodeAt(0), 0)
  return assetUrl(profileImageOptions[seed % profileImageOptions.length])
}
...
function toggleDraftSkyColor(color) {
  const colors = draft.value.skyColors
  if (colors.includes(color)) {
    draft.value.skyColors = colors.filter((selected) => selected !== color)
    return
  }
  if (colors.length >= 3) {
    formError.value = '하늘색은 최대 3가지까지 선택할 수 있어요.'
    return
  }
  draft.value.skyColors = [...colors, color]
  formError.value = ''
}

function addCustomSkyColor(value) {
  toggleDraftSkyColor(value.toUpperCase())
}
...
async function submitComment() {
  ...
  try {
    ...
    selectedPost.value = {
      ...selectedPost.value,
      comments: [...getPostComments(selectedPost.value), comment],
    }
    commentDraft.value = ''
    commentError.value = ''
  } catch (error) {
    commentError.value = error.message || '댓글을 남기지 못했어요. 잠시 후 다시 시도해 주세요.'
  }
}
  ...
  <main class="sky-page">
    ...
    <section class="sky-feed" aria-live="polite" aria-label="오늘의 하늘 사진 피드">
      ...
      <article v-for="post in visiblePosts" :key="post.id" class="sky-card" :class="`sky-card--${post.id % 3}`">
        <button class="sky-card__media" type="button" :aria-label="`${post.title} 상세 보기`" @click="openPostDetail(post)">
          <img :src="assetUrl(post.imageUrl)" :alt="post.title" />
        ...
        <footer>
          <div class="post-tags"><span class="sky-color-tag"><i v-for="color in getPostSkyColors(post)" :key="color" :style="{ backgroundColor: color }"></i>{{ getPostSkyColorLabel(post) }}</span><span>{{ getCloudLabel(post.cloudType) }}</span></div>
          <div class="post-actions">
            <button type="button" :class="{ active: isLiked(post) }" :aria-label="isLiked(post) ? '좋아요 취소' : '좋아요'" :aria-pressed="isLiked(post)" @click="toggleLike(post)"><span class="heart-mark" aria-hidden="true">{{ isLiked(post) ? '♥' : '♡' }}</span><span>{{ likeCount(post) }}</span></button>
            <button type="button" aria-label="댓글 보기" @click="openPostDetail(post)"> <el-icon><ChatDotRound /></el-icon><span>{{ commentCount(post) }}</span></button>
            <button type="button" :class="{ active: isSaved(post) }" :aria-pressed="isSaved(post)" :aria-label="isSaved(post) ? '저장 취소' : '저장'" @click="toggleSave(post)"><el-icon><CollectionTag /></el-icon></button>
          </div>
        </footer>
    ...
    <Transition name="detail-fade">
      <section v-if="selectedPost" class="post-backdrop" aria-label="하늘 사진 상세 보기" @click.self="selectedPost = null">
        <article class="post-detail">
          <figure class="post-detail__photo">
            <img :src="assetUrl(selectedPost.imageUrl)" :alt="selectedPost.title" />
          </figure>
          <section class="post-detail__side">
            <header class="detail-profile">
              <img class="profile-avatar" :src="getProfileImage(selectedPost.author)" :alt="`${selectedPost.author} 프로필 사진`" />
              <div>
                <b>{{ selectedPost.author }}</b>
                <span><el-icon><Location /></el-icon>{{ selectedPost.destination }}</span>
              </div>
            ...
            <div class="detail-scroll-area">
              ...
              <section class="comment-panel" aria-labelledby="comment-title">
                <h3 id="comment-title">댓글 <span>{{ commentCount(selectedPost) }}</span></h3>
                <ul v-if="getPostComments(selectedPost).length" class="comment-list">
                  <li v-for="comment in getPostComments(selectedPost)" :key="comment.id">
                    <img class="profile-avatar profile-avatar--small" :src="getProfileImage(comment.author)" :alt="`${comment.author} 프로필 사진`" />
                    <div><p><b>{{ comment.author }}</b> {{ comment.content }}</p><small>{{ formatRelativeDate(comment.createdAt) }}</small></div>
                  </li>
                </ul>
                <p v-else class="comment-empty">첫 번째로 이 하늘에 말을 건네 보세요.</p>
              </section>
            </div>

            <footer class="detail-footer">
              <div class="detail-actions" aria-label="게시글 반응">
                <button class="detail-heart-button" type="button" :class="{ active: isLiked(selectedPost) }" :aria-label="isLiked(selectedPost) ? '좋아요 취소' : '좋아요'" :aria-pressed="isLiked(selectedPost)" @click="toggleLike(selectedPost)"><span aria-hidden="true">{{ isLiked(selectedPost) ? '♥' : '♡' }}</span></button>
                <button type="button" aria-label="댓글 입력란으로 이동" @click="commentInput?.focus()"><el-icon><ChatDotRound /></el-icon></button>
                <button class="save-button" type="button" :class="{ active: isSaved(selectedPost) }" :aria-label="isSaved(selectedPost) ? '저장 취소' : '저장'" :aria-pressed="isSaved(selectedPost)" @click="toggleSave(selectedPost)"><el-icon><CollectionTag /></el-icon></button>
              </div>
              <p class="detail-like-count">좋아요 <b>{{ likeCount(selectedPost) }}</b>개</p>
              <p class="detail-created-at">{{ formatRelativeDate(selectedPost.createdAt) }}</p>
              <form class="comment-form" @submit.prevent="submitComment">
                <img class="profile-avatar profile-avatar--small" :src="getProfileImage('하늘지기')" alt="내 프로필 사진" />
                <textarea ref="commentInput" v-model="commentDraft" rows="1" maxlength="300" placeholder="댓글 달기…" aria-label="댓글 내용"></textarea>
                <button type="submit" :disabled="!commentDraft.trim() || communityPostStore.isCommenting">{{ communityPostStore.isCommenting ? '등록 중' : '게시' }}</button>
                <p v-if="commentError" role="alert">{{ commentError }}</p>
              </form>
            </footer>
          </section>
        </article>
        ...
        <form class="composer" @submit.prevent="submitPost">
          ...
          <section class="captured-at-field" aria-labelledby="captured-at-label">
            <div class="captured-at-copy">
              ...
              <small>선택 · 달력에서 날짜와 시간을 5분 단위로 선택할 수 있어요.</small>
            </div>
          ...
          <section class="sky-color-field" aria-labelledby="sky-color-label">
            ...
            <p v-if="draft.skyColors.length" class="selected-color"><i v-for="color in draft.skyColors" :key="color" :style="{ backgroundColor: color }"></i>{{ draft.skyColors.map(getSkyColorLabel).join(' · ') }} <small>{{ draft.skyColors.length }}/3</small></p>
          </section>
          <fieldset class="chip-fieldset">
            <legend>구름 양</legend>
...
.filter-menu--taxonomy { width: min(720px, calc(100vw - 64px)); max-height: min(540px, calc(100vh - 170px)); grid-template-columns: repeat(2, minmax(190px, 1fr)); gap: 10px; padding: 13px; overflow-y: auto; overscroll-behavior: contain; }
...
.cloud-tooltip { position: fixed; z-index: 30; top: 50%; right: max(24px, calc((100vw - 1320px) / 2)); left: auto; width: min(245px, calc(100vw - 36px)); max-height: calc(100vh - 36px); overflow: auto; color: #c6dce8; background: #0b1725; border: 1px solid #4a91b8; border-radius: 11px; box-shadow: 0 17px 36px rgba(0, 0, 0, .54); transform: translateY(-50%); }
...
.heart-mark { color: #f4b9c2; font-size: 18px; line-height: 1; }
.post-actions button.active .heart-mark { color: #ff6f88; }
...
.detail-profile { display: flex; align-items: center; gap: 10px; min-height: 70px; padding: 13px 17px; border-bottom: 1px solid #20455e; }
.profile-avatar { display: block; width: 40px; height: 40px; flex: 0 0 auto; object-fit: cover; border: 2px solid #5bc8ef; border-radius: 50%; box-shadow: 0 0 0 2px #0b2438; }
.profile-avatar--small { width: 30px; height: 30px; border-width: 1px; box-shadow: none; }
.detail-profile > div { display: grid; min-width: 0; gap: 3px; }
.detail-profile b { overflow: hidden; color: #effbff; font-size: 13px; text-overflow: ellipsis; white-space: nowrap; }
.detail-profile span { display: inline-flex; align-items: center; gap: 3px; color: #8eb7cc; font-size: 11px; }
.detail-profile span .el-icon { color: #69cef5; }
...
.detail-caption { padding: 24px 18px 21px; border-bottom: 1px solid #20455e; }
.post-detail h2 { margin: 0 0 14px; color: #eefaff; font-size: clamp(18px, 2vw, 23px); line-height: 1.3; letter-spacing: -.03em; }
.detail-caption__content { margin: 0; color: #c0d7e2; font-size: 12px; line-height: 1.7; overflow-wrap: anywhere; }
.detail-captured-at { margin: 15px 0 0; color: #75bce1 !important; font-size: 10px; font-weight: 800; }
.detail-tags { display: flex; flex-wrap: wrap; gap: 6px; padding-top: 0; margin-top: 19px; }
.detail-hashtags { display: flex; flex-wrap: wrap; gap: 6px; padding-top: 0; margin-top: 10px; }
.comment-panel { padding: 17px 18px 19px; }
.comment-panel h3 { display: flex; align-items: baseline; gap: 6px; margin: 0 0 13px; color: #e5f7ff; font-size: 13px; }
.comment-panel h3 span { color: #6ec5ec; font-size: 11px; }
.comment-list { display: grid; gap: 15px; padding: 0; margin: 0; list-style: none; }
.comment-list li { display: grid; grid-template-columns: auto minmax(0, 1fr); gap: 9px; align-items: start; color: #c2d8e3; }
.comment-list li > div { min-width: 0; }
.comment-list p { margin: 1px 0 3px; font-size: 12px; line-height: 1.55; overflow-wrap: anywhere; }
.comment-list b { color: #ecfaff; font-size: 12px; }
.comment-list small { color: #7396aa; font-size: 10px; }
.comment-empty { margin: 0; padding: 13px; color: #819ead !important; background: #061321; border: 1px dashed #27526d; border-radius: 8px; font-size: 11px; text-align: center; }
.detail-footer { padding: 12px 17px 14px; background: #071421; border-top: 1px solid #20455e; }
.detail-actions { display: flex; align-items: center; gap: 10px; }
.detail-actions button { display: grid; width: 27px; height: 27px; padding: 0; color: #c4dce7; background: transparent; border: 0; cursor: pointer; font: inherit; font-size: 21px; place-items: center; }
.detail-actions button:hover, .detail-actions button.active { color: #ff7f94; }
.detail-actions .save-button { margin-left: auto; color: #9bc9dd; font-size: 18px; }
.detail-actions .save-button:hover, .detail-actions .save-button.active { color: #77dbff; }
.detail-heart-button span { line-height: 1; transform: translateY(-1px); }
.detail-like-count { margin: 8px 0 1px; color: #bfd5df; font-size: 11px; }
.detail-like-count b { color: #effaff; }
.detail-created-at { margin: 0 0 10px; color: #6f94a9; font-size: 10px; }
