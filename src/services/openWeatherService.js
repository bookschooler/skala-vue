...
const normalizeAirQuality = (data) => {
  ...
  return {
    ...
    // OpenWeather 기본 Current/Forecast API에는 UV가 없다. 별도 One Call 구독 없이도
    // 화면의 핵심 날씨와 미세먼지는 계속 제공하고, UV는 정보 없음으로 명확히 표시한다.
    uvIndex: null,
  }
}
...
export const fetchWeatherBundle = async (city, { signal } = {}) => {
  ...
  const { hourly, daily } = normalizeForecast(forecastResponse.data)
