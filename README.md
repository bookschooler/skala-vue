...
`.env.local`에는 OpenWeather와 Firebase 웹 설정을 넣습니다.
...
ESLint/Oxlint, 지도 좌표·국가 검색·시간별 예보·날씨 등급을 검증하는 테스트를 통과한 뒤 프로덕션 빌드를 만듭니다.

## GitHub Pages Deployment

`main` 브랜치에 푸시하면 GitHub Actions가 다음 순서로 정적 사이트를 배포합니다.

```text
npm ci → npm run lint → npm run build:production → dist artifact → GitHub Pages
```

GitHub Actions Secrets에는 아래 이름으로 값을 등록합니다.
...
빌드할 때만 Secret을 `VITE_` 환경 변수로 주입합니다. `.env.local`과 빌드 결과물 `dist`는 Git에서 제외합니다. Firebase Google 로그인 사용 시 Firebase Authentication의 **Authorized domains**에 `bookschooler.github.io`를 추가해야 합니다.

---

Made with Vue 3 for the SKALA frontend project.
